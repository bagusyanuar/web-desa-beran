...
use App\Interface\WebPanel\OnlineLetter\ParentIncomeServiceInterface;
...
class ParentIncomeService implements ParentIncomeServiceInterface
{
    public function findAll(ParentIncomeQuery $queryParams): ServiceResponse
    {
        try {
            ...
            return ServiceResponse::statusOK("successfully get certificate parent incomes", $data, $meta);
        } catch (\Throwable $e) {
    ...
    public function createReceipt($id): ServiceResponse
    {
        try {
            //code...
            $certificate = CertificateParentIncome::with(['applicant', 'person', 'parent'])
                ->where('id', '=', $id)
                ->first();
            if (!$certificate) {
                return ServiceResponse::notFound("certificate not found");
            ...
            $pdf = Pdf::loadView('pdf.online-letter.parent-income', [
