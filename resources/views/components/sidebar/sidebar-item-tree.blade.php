...
<div class="w-full" x-data="{ expanded: {{ $isActive ? 'true' : 'false' }} }">
    <div x-on:click="expanded = !expanded;"
        ...
        x-bind:class="expanded ? 'bg-accent-500 text-white' : 'hover:bg-accent-500'">
        ...
        <i data-lucide="{{ $icon }}" class="h-5 aspect-[1/1] transition-all duration-300 ease-in-out"
            x-bind:class="expanded ? 'text-white' : 'text-white/80 group-hover:text-white'"></i>
        <span class="flex-1 font-light transition-all duration-300 ease-in-out"
            x-bind:class="expanded ? 'text-white' : 'text-white/80 group-hover:text-white'">{{ $text }}</span>
        <i data-lucide="chevron-down" class="h-5 aspect-[1/1] transition-all duration-300 ease-in-out"
            x-bind:class="expanded ? 'text-white' : 'text-white/80 group-hover:text-white'"></i>
    </div>
