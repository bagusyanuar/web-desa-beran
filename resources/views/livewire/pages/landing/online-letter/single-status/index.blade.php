<section id="online-letter-single-status" data-component-id="online-letter-single-status" class="w-full">
    <div class="w-full h-[20rem] relative">
        ...
        <div class="w-full h-full absolute top-0 left-0 bg-white/80 flex items-center justify-center">
            <x-container.landing-container class="flex flex-col items-center gap-3">
                <h1 class="text-4xl text-accent-700 font-bold text-center">SURAT KETERANGAN BELUM MENIKAH</h1>
                <p class="text-md text-brand-500 w-1/2 text-center">Dokumen resmi yang dikeluarkan oleh kelurahan atau
                    desa sebagai bukti sah bahwa seseorang
                    tercatat belum pernah menikah sampai dengan tanggal surat diterbitkan.</p>
    ...
    <div class="w-full py-10">
        <x-container.landing-container class="">
            <p class="text-center text-accent-500 text-lg font-bold mb-10">
                FORMULIR SURAT KETERANGAN BELUM MENIKAH PENDUDUK DESA BERAN
            </p>
            <div class="w-5/6 justify-self-center bg-white rounded-xl shadow-xl p-6 border border-neutral-300">
                <div class="w-full">
                    <div
                        ...
                        <div class="flex-1">
                            <p class="text-sm mb-3">Sehubungan dengan pengajuan Surat Keterangan Belum Menikah, bersama ini
                                kami
                    ...
                    <div class="w-full grid grid-cols-2 gap-x-7 gap-y-7">
                        <div class="w-full">
                            ...
                            <x-input.text.text id="name" x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.name" />
                            <template x-if="'name' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.name[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full">
                            ...
                            <x-input.text.text id="nik" x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.nik" />
                            <template x-if="'nik' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.nik[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full">
                            ...
                            <x-input.text.text id="birth-place"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.birthPlace" />
                            <template x-if="'birthPlace' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.birthPlace[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full">
                            ...
                            <x-input.date.datepicker id="date-of-birth" store="SERVICE_SINGLE_STATUS_STORE"
                                stateDate="form.dateOfBirth" format="long" />
                            <template x-if="'dateOfBirth' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.dateOfBirth[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full" wire:ignore>
                            ...
                            <x-select.select2 id="gender" x-init="initSelect({ placeholder: 'pilih jenis kelamin' })"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.gender">
                                <option></option>
                                <option value="male">Laki-Laki</option>
                                <option value="female">Perempuan</option>
                            </x-select.select2>
                            <template x-if="'gender' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.gender[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full" wire:ignore>
                            ...
                            <x-select.select2 id="citizenship" x-init="initSelect({ placeholder: 'pilih kewarganegaraan' })"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.citizenship">
                                <option></option>
                                <option value="indonesia">Indonesia</option>
                                <option value="foreign">Warga Negara Asing</option>
                            </x-select.select2>
                            <template x-if="'citizenship' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.citizenship[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full" wire:ignore>
                            ...
                            <x-select.select2 id="religion" x-init="initSelect({ placeholder: 'pilih agama' })"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.religion">
                                <option></option>
                            ...
                            <template x-if="'religion' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.religion[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full">
                            ...
                            <x-input.text.text id="job" x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.job" />
                        </div>
                        <div class="w-full col-span-2">
                            ...
                            <x-input.text.textarea id="address" rows="3"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.address" />
                            <template x-if="'address' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.address[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="w-full grid grid-cols-2 gap-x-7 gap-y-7 mb-10">
                        <div class="w-full">
                            ...
                            <x-input.text.text id="applicant_name"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.applicantName" />
                            <template x-if="'applicantName' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.applicantName[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full">
                            ...
                            <x-input.text.text id="applicant_name"
                                x-model="$store.SERVICE_SINGLE_STATUS_STORE.form.applicantPhone" />
                            <template x-if="'applicantPhone' in $store.SERVICE_SINGLE_STATUS_STORE.formValidator">
                                <x-label.validator>
                                    <span
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.formValidator.applicantPhone[0]"></span>
                                </x-label.validator>
                            </template>
                        </div>
                        <div class="w-full col-span-2 flex justify-center items-center" wire:ignore>
                            <x-captcha.recaptcha store="SERVICE_SINGLE_STATUS_STORE" stateData="captchaToken" />
                        </div>
                    </div>

                    <button class="w-full rounded-lg text-md text-white py-2.5 bg-brand-500"
                        x-on:click="$store.SERVICE_SINGLE_STATUS_STORE.send()">
                        <span>Kirim</span>
                    </button>
                </div>
            </div>
    ...
    <div>
        <div x-cloak x-show="$store.SERVICE_SINGLE_STATUS_STORE.showReceipt"
        ...
        <div x-cloak x-show="$store.SERVICE_SINGLE_STATUS_STORE.showReceipt"
            ...
            <div class="bg-white rounded-lg shadow-lg w-[40rem] p-4">
                <p class="text-lg text-accent-500 text-center font-bold mb-5">SURAT KETERANGAN BELUM MENIKAH BERHASIL
                    DIBUAT
                </p>
                            ...
                            <tr>
                                ...
                                <td>
                                    <span class="text-sm text-neutral-700"
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.receiptData.referenceNumber"></span>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                ...
                                <td>
                                    <span class="text-sm text-neutral-700"
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.receiptData.applicantName"></span>
                                </td>
                            </tr>
                            <tr>
                                <td>
                                ...
                                <td>
                                    <span class="text-sm text-neutral-700"
                                        x-text="$store.SERVICE_SINGLE_STATUS_STORE.receiptData.applicantPhone"></span>
                                </td>
                            </tr>
                        </table>
                        <div class="w-full flex items-center gap-1">
                            <button x-on:click="$store.SERVICE_SINGLE_STATUS_STORE.closeReceipt()"
                            ...
                            <button
                                ...
                                x-on:click="$store.SERVICE_SINGLE_STATUS_STORE.download()">
    ...
    <x-alert.confirmation onAccept="$store.SERVICE_SINGLE_STATUS_STORE.onAccept()">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengirim permohonan <span
                class="font-semibold">Surat Keterangan Belum Menikah</span>. Pastikan data yang anda isi sudah
            lengkap dan
                        ...
                        .then(response => {
                            ...
                            switch (status) {
                                case 201:
                                    this.formClear();
                                    const code = data['url'];
                                    const singleStatus = data['single_status'];
                                    const {
                                        name, phone
                                    } = data['applicant']
                                    const referenceNumber = singleStatus['reference_number'];
