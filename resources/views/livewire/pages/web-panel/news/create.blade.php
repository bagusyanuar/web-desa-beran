<section id="news-create" data-component-id="news-create" class="w-full">
    <div class="mb-5 flex items-start gap-2">
        ...
        <div>
            <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Berita</p>
            <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola berita atau artikel desa beran.</p>
        </div>
    </div>
    <div class="w-full p-6 bg-white shadow-2xl rounded-lg border-t-4 border-accent-500 mb-6">
        ...
        <div class="w-full grid grid-cols-2 gap-x-6 gap-y-6">
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="judul berita" id="title"
                    x-model="$store.SERVICE_NEWS_CREATE_STORE.form.title" />
                <template x-if="'title' in $store.SERVICE_NEWS_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEWS_CREATE_STORE.formValidator.title[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2" wire:ignore>
                ...
                <x-input.text.summernote height="300" id="description" placeholder="isi berita"
                    x-model="$store.SERVICE_NEWS_CREATE_STORE.form.description" />
                <template x-if="'description' in $store.SERVICE_NEWS_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEWS_CREATE_STORE.formValidator.description[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.file.dropzone store="SERVICE_NEWS_CREATE_STORE" stateComponent="thumbnailDropper"
                    class="!h-12"></x-input.file.dropzone>
                <template x-if="'thumbnail' in $store.SERVICE_NEWS_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEWS_CREATE_STORE.formValidator.thumbnail[0]"></span>
                    </x-label.validator>
                </template>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_NEWS_CREATE_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Pembuatan Berita Baru"
        onAccept="$store.SERVICE_NEWS_CREATE_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Pembuatan
                Berita Baru</span>. Pastikan data yang anda isi sudah
            lengkap dan
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_NEWS_CREATE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="news-create"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    const uploadThumbnailImage = this.thumbnailDropper.files.map(file => {
                    ...
                    switch (status) {
                        case 201:
                            this.toastStore.success("Berhasil menambahkan berita atau artikel", 2000,
                                function() {
