<section id="micro-business-detail" data-component-id="micro-business-detail" class="w-full">
    ...
    <div class="w-full p-6 bg-white shadow-2xl rounded-lg border-t-4 border-accent-500 mb-6">
        ...
        <div class="w-full grid grid-cols-2 gap-x-6 gap-y-6">
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="nama produk umkm" id="name"
                    x-model="$store.SERVICE_MICRO_BUSINESS_DETAIL_STORE.form.title" />
        ...
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_MICRO_BUSINESS_DETAIL_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Produk UMKM"
        onAccept="$store.SERVICE_MICRO_BUSINESS_DETAIL_STORE.onConfirm()" acceptText="Konfrimasi">
        ...
        document.addEventListener('alpine:init', () => {
            ...
            const STORE_PROPS = {
                ...
                async onConfirm() {
                    ...
                    const uploadOwnerImage = this.ownerPictureDropper.files
                    ...
                    const uploadProductImage = this.productPictureDropper.files
                        .filter(file => file instanceof File)
                        .map(file => {
                            return new Promise((resolve, reject) => {
                    ...
                    switch (status) {
                        case 200:
                            this.toastStore.success("Berhasil merubah data produk UMKM", 2000,
                                function() {
                                    window.location.href = '/web-panel/produk-umkm';
                                });
                            break;
                        case 422:
                        ...
                        switch (status) {
                            case 200:
                                if (data) {
                                    ...
                                    const ownerAddress = address['address'];
                                    const ownerContact = contact['value'];
                                    const productImage = image['image'];

                                    this.form.title = title;
                                    this.form.description = description;
                                    this.form.ownerName = ownerName;
                                    this.form.ownerContact = ownerContact;
                                    this.form.ownerAddress = ownerAddress
                                    this.form.ownerDescription = ownerDescription;
                                    ...
                                    if (ownerImage) {
                                        this.ownerPictureDropper.emit("addedfile", mockFileOwner);
                                        this.ownerPictureDropper.emit("thumbnail", mockFileOwner,
                                            ownerImage);
                                        this.ownerPictureDropper.emit("complete", mockFileOwner);
                                        this.ownerPictureDropper.files.push(mockFileOwner);
                                    }

                                    if (productImage) {
                                        this.productPictureDropper.emit("addedfile", mockFileProduct);
                                        this.productPictureDropper.emit("thumbnail", mockFileProduct,
                                            productImage);
                                        this.productPictureDropper.emit("complete", mockFileProduct);
                                        this.productPictureDropper.files.push(mockFileProduct);
                                    }
