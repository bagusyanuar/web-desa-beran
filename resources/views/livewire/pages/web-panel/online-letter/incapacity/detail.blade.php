<section id="online-letter-incapacity-detail" data-component-id="online-letter-incapacity-detail" class="w-full">
    <div class="mb-5 flex items-start gap-2">
        <a href="{{ route('web-panel.online-letter.incapacity') }}"
        ...
        <div>
            <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Surat Keterangan Tidak Mampu</p>
            <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola surat keterangan tidak mampu.</p>
        </div>
    </div>
            ...
            @if ($data->status === App\Commons\Enum\CertificateStatus::Created->value)
                ...
                <div class="w-full mb-3" wire:ignore>
                    <x-select.select2 id="status" x-init="initSelect({ placeholder: 'pilih status konfirmasi' })"
                        x-model="$store.SERVICE_INCAPACITY_DETAIL_STORE.form.status">
                        <option></option>
                        <option value="accept">Terima</option>
                        <option value="denied">Tolak</option>
                    </x-select.select2>
                    <template x-if="'status' in $store.SERVICE_INCAPACITY_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_INCAPACITY_DETAIL_STORE.formValidator.status[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div x-cloak x-show="$store.SERVICE_INCAPACITY_DETAIL_STORE.form.status === 'denied'" class="w-full"
                    wire:ignore>
                    ...
                    <x-input.text.textarea id="address" rows="3" class="!text-sm"
                        x-model="$store.SERVICE_INCAPACITY_DETAIL_STORE.form.reason" />
                    <template x-if="'reason' in $store.SERVICE_INCAPACITY_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_INCAPACITY_DETAIL_STORE.formValidator.reason[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div class="w-full border-b border-neutral-300 my-3">
                </div>
                <button x-on:click="$store.SERVICE_INCAPACITY_DETAIL_STORE.confirm()"
    ...
    <x-alert.confirmation onAccept="$store.SERVICE_INCAPACITY_DETAIL_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Surat
                Keterangan Tidak Mampu</span>. Pastikan data yang anda isi sudah
            lengkap dan
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_INCAPACITY_DETAIL_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                                '[data-component-id="online-letter-incapacity-detail"]')
                            ?.getAttribute(
                                'wire:id');
