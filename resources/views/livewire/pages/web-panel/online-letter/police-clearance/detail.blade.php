<section id="online-letter-police-clearance-detail" data-component-id="online-letter-police-clearance-detail"
    ...
    <div class="w-full flex items-start gap-4">
        <div class="flex-1 p-6 bg-white shadow-2xl rounded-lg border-t-4 border-accent-500">
                ...
                <table class="border-collapse w-full text-sm font-semibold">
                    <tr class="even:bg-white odd:bg-brand-50">
                        ...
                        <td class="px-3 py-2.5">
                            <span class="uppercase">{{ $data->person->name }}</span>
                        </td>
                    </tr>
                    <tr class="even:bg-white odd:bg-brand-50">
                        <td class="px-3 py-2.5">
            ...
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm font-bold text-neutral-700 mb-3">B. Catatan Keterangan</p>
            </div>
            <div class="w-full overflow-hidden rounded-lg border border-neutral-300 mb-5">
                <table class="border-collapse w-full text-sm font-semibold">
                    @foreach ($data->descriptions as $key => $description)
                        <tr class="even:bg-white odd:bg-brand-50">
                            <td class="px-3 py-2.5 w-14 text-center">
            ...
            @if ($data->status === App\Commons\Enum\CertificateStatus::Created->value)
                ...
                <div class="w-full mb-3" wire:ignore>
                    <x-select.select2 id="status" x-init="initSelect({ placeholder: 'pilih status konfirmasi' })"
                        x-model="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.form.status">
                        <option></option>
                        <option value="accept">Terima</option>
                        <option value="denied">Tolak</option>
                    </x-select.select2>
                    <template x-if="'status' in $store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.formValidator.status[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div x-cloak x-show="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.form.status === 'denied'"
                    ...
                    <x-input.text.textarea id="address" rows="3" class="!text-sm"
                        x-model="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.form.reason" />
                    <template x-if="'reason' in $store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.formValidator.reason[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div class="w-full border-b border-neutral-300 my-3">
                </div>
                <button x-on:click="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.confirm()"
            ...
            @if (
                ...
                <div class="w-full flex flex-col gap-2">
                    ...
                    @if ($data->status === App\Commons\Enum\CertificateStatus::Pending->value)
                        <button x-on:click="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.finish()"
    ...
    <x-alert.confirmation onAccept="$store.SERVICE_POLICE_CLEARANCE_DETAIL_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Surat
                Keterangan Catatan Kepolisian</span>. Pastikan data yang anda isi sudah
            lengkap dan
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                                '[data-component-id="online-letter-police-clearance-detail"]')
                            ?.getAttribute(
                                'wire:id');
