<section id="online-letter-parent-income-detail" data-component-id="online-letter-parent-income-detail" class="w-full">
    <div class="mb-7 flex items-start gap-3">
        <a href="{{ route('web-panel.online-letter.parent-income') }}"
    ...
    <div class="w-full flex items-start gap-4">
        <div class="flex-1 p-6 bg-white border border-neutral-300 shadow-xl rounded-lg">
            ...
            <div class="flex items-center justify-between mb-3">
                <p class="text-sm font-bold text-neutral-700 mb-3">B. Data Orang Tua / Wali</p>
            </div>
            ...
            @if ($data->status === App\Commons\Enum\CertificateStatus::Created->value)
                ...
                <div class="w-full mb-3" wire:ignore>
                    <x-select.select2 id="status" x-init="initSelect({ placeholder: 'pilih status konfirmasi' })"
                        x-model="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.form.status">
                        <option></option>
                        <option value="accept">Terima</option>
                        <option value="denied">Tolak</option>
                    </x-select.select2>
                    <template x-if="'status' in $store.SERVICE_PARENT_INCOME_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.formValidator.status[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div x-cloak x-show="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.form.status === 'denied'"
                    ...
                    <x-input.text.textarea id="address" rows="3" class="!text-sm"
                        x-model="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.form.reason" />
                    <template x-if="'reason' in $store.SERVICE_PARENT_INCOME_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.formValidator.reason[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div class="w-full border-b border-neutral-300 my-3">
                </div>
                <button x-on:click="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.confirm()"
            ...
            @if (
                ...
                <div class="w-full flex flex-col gap-2">
                    <div class="flex items-center gap-2">
                        ...
                        <button x-on:click="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.download('{{ $id }}')"
                    ...
                    @if ($data->status === App\Commons\Enum\CertificateStatus::Pending->value)
                        <button x-on:click="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.finish()"
    ...
    <x-alert.confirmation onAccept="$store.SERVICE_PARENT_INCOME_DETAIL_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Surat
                Keterangan Penghasilan Orang Tua</span>. Pastikan data yang anda isi sudah
            lengkap dan
