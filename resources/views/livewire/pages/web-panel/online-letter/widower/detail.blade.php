<section id="online-letter-widower-detail" data-component-id="online-letter-widower-detail" class="w-full">
    <div class="mb-5 flex items-start gap-2">
        <a href="{{ route('web-panel.online-letter.widower') }}"
        ...
        <div>
            <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Surat Keterangan Duda</p>
            <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola surat keterangan duda.</p>
        </div>
    </div>
            ...
            @if ($data->status === App\Commons\Enum\CertificateStatus::Created->value)
                ...
                <div class="w-full mb-3" wire:ignore>
                    <x-select.select2 id="status" x-init="initSelect({ placeholder: 'pilih status konfirmasi' })"
                        x-model="$store.SERVICE_WIDOWER_DETAIL_STORE.form.status">
                        <option></option>
                        <option value="accept">Terima</option>
                        <option value="denied">Tolak</option>
                    </x-select.select2>
                    <template x-if="'status' in $store.SERVICE_WIDOWER_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_WIDOWER_DETAIL_STORE.formValidator.status[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div x-cloak x-show="$store.SERVICE_WIDOWER_DETAIL_STORE.form.status === 'denied'" class="w-full"
                    wire:ignore>
                    ...
                    <x-input.text.textarea id="address" rows="3" class="!text-sm"
                        x-model="$store.SERVICE_WIDOWER_DETAIL_STORE.form.reason" />
                    <template x-if="'reason' in $store.SERVICE_WIDOWER_DETAIL_STORE.formValidator">
                        <x-label.validator>
                            <span x-text="$store.SERVICE_WIDOWER_DETAIL_STORE.formValidator.reason[0]"></span>
                        </x-label.validator>
                    </template>
                </div>
                <div class="w-full border-b border-neutral-300 my-3">
                </div>
                <button x-on:click="$store.SERVICE_WIDOWER_DETAIL_STORE.confirm()"
            ...
            @if (
                ...
                <div class="w-full flex flex-col gap-2">
                    <div class="flex items-center gap-2">
                        ...
                        <button x-on:click="$store.SERVICE_WIDOWER_DETAIL_STORE.download('{{ $id }}')"
                    ...
                    @if ($data->status === App\Commons\Enum\CertificateStatus::Pending->value)
                        <button x-on:click="$store.SERVICE_WIDOWER_DETAIL_STORE.finish()"
    ...
    <x-alert.confirmation onAccept="$store.SERVICE_WIDOWER_DETAIL_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Surat
                Keterangan Duda</span>. Pastikan data yang anda isi sudah
            lengkap dan
