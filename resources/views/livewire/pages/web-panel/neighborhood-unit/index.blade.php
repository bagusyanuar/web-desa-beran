<section id="neighborhood-unit" data-component-id="neighborhood-unit" class="w-full">
    <div class="mb-5">
    ...
    <div class="w-full bg-white shadow-2xl p-6 rounded-lg border-t-4 border-accent-500">
        <div class="flex items-center justify-between mb-3">
            <span class="text-sm font-bold text-neutral-700 uppercase">Data RT</span>
            <div class="flex items-center gap-1">
                <x-table.search store="SERVICE_NEIGHBORHOOD_UNIT_STORE" dispatcher="findAll" />
                <a href="{{ route('web-panel.rt.new') }}"
        ...
        <x-table.table store="SERVICE_NEIGHBORHOOD_UNIT_STORE">
            <x-table.thead>
                <x-table.th width="w-[220px]" align="center">
                    <span class="text-sm text-neutral-700">Dusun</span>
                </x-table.th>
                <x-table.th width="w-[150px]" align="center">
                    <span class="text-sm text-neutral-700">RW</span>
                </x-table.th>
                <x-table.th width="w-[180px]" align="center">
                    <span class="text-sm text-neutral-700">Nomor</span>
                </x-table.th>
                <x-table.th>
                    <span class="text-sm text-neutral-700">Ketua</span>
                </x-table.th>
                <x-table.th width="w-[180px]" align="center">
            ...
            <x-table.tbody>
                <template x-for="(v, index) in data" :key="index">
                    <x-table.row>
                        <x-table.td width="w-[220px]" align="center">
                            <span class="text-sm text-neutral-700 text-center"
                                x-text="v.community_unit?.village?.name"></span>
                        </x-table.td>
                        <x-table.td width="w-[150px]" align="center">
                            <span class="text-sm text-neutral-700 text-center"
                                x-text="v.community_unit?.code"></span>
                        </x-table.td>
                        ...
                        <x-table.td>
                            <span class="text-sm text-neutral-700" x-text="v.leader_name"></span>
                        </x-table.td>
                        <x-table.td width="w-[180px]" align="center">
                            <span class="text-sm text-neutral-700 text-center"
                                x-text="v.leader_contact"></span>
                        </x-table.td>
                            ...
                            <x-popper.popper>
                                ...
                                <div x-bind="popperContentBind"
                                    ...
                                    <div class="flex flex-col py-1 justify-start items-start">
                                        <div x-on:click="$store.SERVICE_NEIGHBORHOOD_UNIT_STORE.goToDetail(v.id)"
                                        ...
                                        <div x-on:click="$store.SERVICE_NEIGHBORHOOD_UNIT_STORE.confirm(v.id)"
                                            class="flex items-center justify-start gap-1 w-full text-xs px-2 py-1.5 cursor-pointer hover:bg-neutral-50">
                                            <div wire:ignore>
        ...
        <x-table.pagination store="SERVICE_NEIGHBORHOOD_UNIT_STORE" dispatcher="findAll" />
    </div>
    <x-alert.confirmation title="Konfirmasi Hapus Berita" onAccept="$store.SERVICE_NEIGHBORHOOD_UNIT_STORE.onConfirm()"
        ...
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span
                ...
                rt</span>. Pastikan data yang anda pilih sudah
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_NEIGHBORHOOD_UNIT_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="neighborhood-unit"]')?.getAttribute(
                            'wire:id');
                ...
                goToDetail(id) {
                    window.location.href = "/web-panel/rt/" + id + "/edit";
                },
                confirm(id) {
                    this.selectedId = id;
