<section id="neighborhood-unit-create" data-component-id="neighborhood-unit-create" class="w-full">
    ...
    <div class="w-full p-6 bg-white shadow-2xl rounded-lg border-t-4 border-accent-500 mb-6">
        ...
        <div class="w-full grid grid-cols-2 gap-x-6 gap-y-6">
            <div class="w-full col-span-2" wire:ignore>
                ...
                <x-select.select2 id="community-unit" x-init="initSelect({ placeholder: 'pilih rw' })"
                    x-model="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.form.communityUnitId">
                    <option></option>
                ...
                <template x-if="'communityUnitId' in $store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator.communityUnitId[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="nomor rt" id="code"
                    x-model="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.form.code" />
                <template x-if="'code' in $store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator.code[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="ketua rt" id="leader-name"
                    x-model="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.form.leaderName" />
                <template x-if="'leaderName' in $store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator.leaderName[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="ketua rw" id="leader-contact"
                    x-model="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.form.leaderContact" />
                <template x-if="'leaderContact' in $store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.formValidator.leaderContact[0]"></span>
                    </x-label.validator>
                </template>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Pembuatan Berita Baru"
        onAccept="$store.SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE.onConfirm()" acceptText="Konfrimasi">
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_NEIGHBORHOOD_UNIT_CREATE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="neighborhood-unit-create"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    switch (status) {
                        case 201:
                            this.toastStore.success("Berhasil menambahkan data RT", 2000,
                                function() {
