<section id="village" data-component-id="village" class="w-full">
    <div class="mb-5">
    ...
    <div class="w-full bg-white shadow-2xl p-6 rounded-lg border-t-4 border-accent-500">
        <div class="flex items-center justify-between mb-3">
            <span class="text-sm font-bold text-neutral-700 uppercase">Data Dusun</span>
            <div class="flex items-center gap-1">
                <x-table.search store="SERVICE_VILLAGE_STORE" dispatcher="findAll" />
                <a href="{{ route('web-panel.village.new') }}"
        ...
        <x-table.table store="SERVICE_VILLAGE_STORE">
            <x-table.thead>
                <x-table.th>
                    <span class="text-sm text-neutral-700">Nama</span>
                </x-table.th>
                <x-table.th>
                            ...
                            <x-popper.popper>
                                ...
                                <div x-bind="popperContentBind"
                                    ...
                                    <div class="flex flex-col py-1 justify-start items-start">
                                        <div x-on:click="$store.SERVICE_VILLAGE_STORE.goToDetail(v.id)"
                                        ...
                                        <div x-on:click="$store.SERVICE_VILLAGE_STORE.confirm(v.id)"
                                            class="flex items-center justify-start gap-1 w-full text-xs px-2 py-1.5 cursor-pointer hover:bg-neutral-50">
                                            <div wire:ignore>
        ...
        <x-table.pagination store="SERVICE_VILLAGE_STORE" dispatcher="findAll" />
    </div>
    <x-alert.confirmation title="Konfirmasi Hapus Berita" onAccept="$store.SERVICE_VILLAGE_STORE.onConfirm()"
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_VILLAGE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="village"]')?.getAttribute(
                            'wire:id');
                ...
                goToDetail(id) {
                    window.location.href = "/web-panel/dusun/" + id + "/edit";
                },
                confirm(id) {
                    this.selectedId = id;
