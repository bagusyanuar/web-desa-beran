<section id="village-update" data-component-id="village-update" class="w-full">
    <div class="mb-5 flex items-start gap-2">
        <a href="{{ route('web-panel.village') }}"
        ...
        <div>
            <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Dusun</p>
            <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola data dusun pada desa beran.</p>
        </div>
    </div>
    <div class="w-full p-6 bg-white shadow-2xl rounded-lg border-t-4 border-accent-500 mb-6">
        <p class="text-sm font-bold text-neutral-700 mb-0">Informasi Dusun</p>
        ...
        <div class="w-full grid grid-cols-2 gap-x-6 gap-y-6">
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="nama dusun" id="name"
                    x-model="$store.SERVICE_VILLAGE_UPDATE_STORE.form.name" />
                <template x-if="'name' in $store.SERVICE_VILLAGE_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_VILLAGE_UPDATE_STORE.formValidator.name[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="ketua dusun" id="leader-name"
                    x-model="$store.SERVICE_VILLAGE_UPDATE_STORE.form.leaderName" />
                <template x-if="'leaderName' in $store.SERVICE_VILLAGE_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_VILLAGE_UPDATE_STORE.formValidator.leaderName[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="kontak" id="leader-contact"
                    x-model="$store.SERVICE_VILLAGE_UPDATE_STORE.form.leaderContact" />
                <template x-if="'leaderContact' in $store.SERVICE_VILLAGE_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_VILLAGE_UPDATE_STORE.formValidator.leaderContact[0]"></span>
                    </x-label.validator>
                </template>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_VILLAGE_UPDATE_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Pembuatan Berita Baru"
        onAccept="$store.SERVICE_VILLAGE_UPDATE_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Perubahan
                Dusun</span>. Pastikan data yang anda isi sudah
            lengkap dan
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_VILLAGE_UPDATE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="village-update"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    switch (status) {
                        case 200:
                            this.toastStore.success("Berhasil merubah data dusun", 2000,
                                function() {
                                    window.location.href = '/web-panel/dusun';
                                });
                            break;
                        case 422:
