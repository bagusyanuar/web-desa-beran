<section id="potention" data-component-id="potention" class="w-full">
    <div class="mb-5">
        <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Potensi Desa</p>
        <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola profil potensi desa beran.</p>
    </div>
    <div class="w-full bg-white shadow-2xl p-6 rounded-lg border-t-4 border-accent-500">
        <div class="w-full mb-6" wire:ignore>
            <x-label.label for="about-description">
                <span>Deskripsi Potensi Desa Beran</span>
            </x-label.label>
            <x-input.text.summernote id="about-description" height="350"
                x-model="$store.SERVICE_POTENTION_STORE.form.description" />
            <template x-if="'description' in $store.SERVICE_POTENTION_STORE.formValidator">
                <x-label.validator>
                    <span x-text="$store.SERVICE_POTENTION_STORE.formValidator.description[0]"></span>
                </x-label.validator>
            </template>
        </div>
        <div class="w-full border-b border-neutral-300 my-3"></div>
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_POTENTION_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Perubahan Profil" onAccept="$store.SERVICE_POTENTION_STORE.onConfirm()"
        ...
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Perubahan
                Profil Potensi DESA BERAN</span>. Pastikan data yang anda isi sudah
            lengkap dan
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_POTENTION_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="potention"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    switch (status) {
                        case 200:
                            this.toastStore.success("Berhasil merubah data potensi desa", 2000,
                                function() {
