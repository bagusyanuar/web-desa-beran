<section id="staff" data-component-id="staff" class="w-full">
        ...
        <x-table.table store="SERVICE_STAFF_STORE">
            <x-table.thead>
                <x-table.th width="w-[200px]" align="center">
                    <span class="text-sm text-neutral-700">Foto</span>
                </x-table.th>
                <x-table.th width="w-[200px]" align="center">
                    <span class="text-sm text-neutral-700">Jabatan</span>
                </x-table.th>
                <x-table.th>
                            ...
                            <x-popper.popper>
                                ...
                                <div x-bind="popperContentBind"
                                    ...
                                    <div class="flex flex-col py-1 justify-start items-start">
                                        <div x-on:click="$store.SERVICE_STAFF_STORE.goToDetail(v.id)"
                                        ...
                                        <div x-on:click="$store.SERVICE_STAFF_STORE.confirm(v.id)"
                                            class="flex items-center justify-start gap-1 w-full text-xs px-2 py-1.5 cursor-pointer hover:bg-neutral-50">
                                            <div wire:ignore>
    ...
    <x-alert.confirmation title="Konfirmasi Hapus Berita" onAccept="$store.SERVICE_STAFF_STORE.onConfirm()"
        ...
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span
                ...
                Perangkat Desa</span>. Pastikan data yang anda pilih sudah
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_STAFF_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="staff"]')?.getAttribute(
                            'wire:id');
                ...
                goToDetail(id) {
                    window.location.href = "/web-panel/perangkat-desa/" + id + "/edit";
                },
                confirm(id) {
                    this.selectedId = id;
                ...
                onConfirm() {
                    ...
                    this.component.$wire.call('delete', this.selectedId)
                        .then(response => {
                            ...
                            if (status === 200) {
                                let self = this;
                                this.toastStore.success("Berhasil menghapus data perangkat desa", 2000,
                                    function() {
                                        self.findAll();
                                    });
                            } else {
