<section id="library-update" data-component-id="library-update" class="w-full">
    <div class="mb-7 flex items-start gap-3">
        ...
        <div>
            <p class="text-xl text-neutral-700 font-bold">Perpustakaan Online</p>
            <p class="text-md text-neutral-500">Halaman ini digunakan untuk mengelola perpustakaan online desa beran.</p>
        </div>
    </div>
    <div class="w-full p-6 bg-white border border-neutral-300 shadow-xl rounded-lg mb-6">
        ...
        <div class="w-full grid grid-cols-2 gap-x-6 gap-y-6">
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="judul" id="title"
                    x-model="$store.SERVICE_LIBRARY_UPDATE_STORE.form.title" />
                <template x-if="'title' in $store.SERVICE_LIBRARY_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_LIBRARY_UPDATE_STORE.formValidator.title[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.text.text placeholder="nama pengarang" id="author-name"
                    x-model="$store.SERVICE_LIBRARY_UPDATE_STORE.form.authorName" />
                <template x-if="'authorName' in $store.SERVICE_LIBRARY_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_LIBRARY_UPDATE_STORE.formValidator.authorName[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.file.dropzone store="SERVICE_LIBRARY_UPDATE_STORE" stateComponent="imageDropper"
                    class="!h-12"></x-input.file.dropzone>
                <template x-if="'image' in $store.SERVICE_LIBRARY_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_LIBRARY_UPDATE_STORE.formValidator.image[0]"></span>
                    </x-label.validator>
                </template>
            </div>
            <div class="w-full col-span-2">
                ...
                <x-input.file.dropzone store="SERVICE_LIBRARY_UPDATE_STORE" stateComponent="fileDropper"
                    class="!h-12"></x-input.file.dropzone>
                <template x-if="'file' in $store.SERVICE_LIBRARY_UPDATE_STORE.formValidator">
                    <x-label.validator>
                        <span x-text="$store.SERVICE_LIBRARY_UPDATE_STORE.formValidator.file[0]"></span>
                    </x-label.validator>
                </template>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_LIBRARY_UPDATE_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Pembuatan Berita Baru"
        onAccept="$store.SERVICE_LIBRARY_UPDATE_STORE.onConfirm()" acceptText="Konfrimasi">
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_LIBRARY_UPDATE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="library-update"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    switch (status) {
                        case 200:
                            this.toastStore.success("Berhasil merubah perpustakaan online", 2000,
                                function() {
                                    window.location.href = '/web-panel/perpustakaan-online';
                                });
                            break;
                        case 422:
                        ...
                        switch (status) {
                            case 200:
                                if (data) {
                                    ...
                                    this.form.authorName = author_name;
