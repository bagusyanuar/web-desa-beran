<section id="setting-landing-title" data-component-id="setting-landing-title" class="w-full">
    <div class="mb-5">
        <p class="text-lg text-neutral-700 font-semibold leading-[1.2]">Teks Ucapan</p>
        <p class="text-sm text-neutral-500">Halaman ini digunakan untuk mengelola teks ucapan pada website desa beran.
        </p>
    </div>
    <div class="w-full bg-white shadow-2xl p-6 rounded-lg border-t-4 border-accent-500">
        <div class="w-full mb-3">
            ...
            <x-input.text.textarea id="title" rows="3" class="!text-sm"
                x-model="$store.SERVICE_SETTING_LANDING_TITLE_STORE.form.title" />
        </div>
        <div class="w-full">
            <x-label.label for="sub-title">
                <span>Sub Judul</span>
                <span class="text-red-500 text-sm italic">*</span>
            </x-label.label>
            <x-input.text.textarea id="sub-title" rows="3" class="!text-sm"
                x-model="$store.SERVICE_SETTING_LANDING_TITLE_STORE.form.subTitle" />
        </div>
        <div class="w-full border-b border-neutral-300 my-3"></div>
        <div class="flex items-center justify-end">
            <button x-on:click="$store.SERVICE_SETTING_LANDING_TITLE_STORE.confirm()"
    ...
    <x-alert.confirmation title="Konfirmasi Perubahan Profil"
        onAccept="$store.SERVICE_SETTING_LANDING_TITLE_STORE.onConfirm()" acceptText="Konfrimasi">
        <p class="text-sm text-neutral-700 text-justify">Anda akan mengkonfirmasi <span class="font-semibold">Perubahan
                Teks Ucapan Website DESA BERAN</span>. Pastikan data yang anda isi sudah
            lengkap dan
...
@push('scripts')
    <script>
        document.addEventListener('alpine:init', () => {
            const STORE_NAME = 'SERVICE_SETTING_LANDING_TITLE_STORE';
            const STORE_PROPS = {
                component: null,
                alertStore: null,
                ...
                init: function() {
                    Livewire.hook('component.init', ({
                        ...
                        const componentID = document.querySelector(
                            '[data-component-id="setting-landing-title"]')?.getAttribute(
                            'wire:id');
                ...
                async onConfirm() {
                    ...
                    switch (status) {
                        case 200:
                            this.toastStore.success("Berhasil mengganti judul latar", 2000,
                                function() {
                ...
                getSetting() {
                    ...
                    this.component.$wire.call('getSetting').then(response => {
                        ...
                        if (data) {
                            ...
                            if (landing_title) {
                                this.form.title = landing_title;
                            }
                            if (landing_sub_title) {
                                this.form.subTitle = landing_sub_title;
                            }
                        }
